...
export default function ChatInterface({ 
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
            ...
            if (line.startsWith('data: ') && line !== 'data: [DONE]') {
              try {
                ...
                if (data.type === 'metadata') {
                  ...
                  // We could store eval_json in state if we want to display it
                  setTurns(prev => {
                    const newTurns = [...prev];
      ...
      // Speak AI response
      if (window.speechSynthesis && streamedContent) {
        const cleanText = streamedContent.replace(/\*/g, '');
  ...
  return (
    <div className="flex flex-col h-full bg-gray-950">
          ...
          <div key={i} className={`flex ${turn.speaker === 'candidate' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-3xl rounded-2xl p-5 shadow-sm ${
              ...
              <div className="text-xs opacity-50 mb-1 flex justify-between items-center">
                <span>{turn.speaker === 'candidate' ? 'You' : 'AI Interviewer'}</span>
      ...
      <div className="p-4 bg-gray-900 border-t border-gray-800">
        {status === 'completed' ? (
          <div className="text-center p-4 text-green-400 font-medium">
            Interview complete! Your rubric is being generated.
          </div>
        ) : (
